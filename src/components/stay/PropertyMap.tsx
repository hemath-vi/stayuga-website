...
interface PropertyMapProps {
  ...
  /** Optional pre-built embed URL (e.g. `location.mapEmbedUrl` from the API) */
  mapEmbedUrl?: string;
}
 ...
 *
 * Uses the keyless `output=embed` endpoint by default. If you have a Maps
 * Embed API key, set NEXT_PUBLIC_GOOGLE_MAPS_KEY and it will be used instead.
 */
export function PropertyMap({ name, address, coordinates, mapEmbedUrl }: PropertyMapProps) {
  ...
  const embedSrc =
    mapEmbedUrl ??
    (apiKey
      ? `https://www.google.com/maps/embed/v1/place?key=${apiKey}&q=${lat},${lng}&zoom=14`
      : `https://www.google.com/maps?q=${lat},${lng}&z=14&hl=en&output=embed`);
    ...
    <div className="overflow-hidden rounded-2xl border border-stone-200 bg-white">
      ...
      <div className="relative h-56 w-full bg-stone-100 sm:h-72 lg:h-80">
        {loaded ? (
          ...
          >
            ...
            />
            <span className="relative flex h-12 w-12 items-center justify-center rounded-full bg-[#48182E] text-white shadow-lg transition-transform group-hover:scale-105">
      ...
      <div className="p-4 sm:p-5">
        <div className="flex items-start gap-3">
          <MapPin size={18} strokeWidth={1.6} className="mt-0.5 shrink-0 text-[#8c7456]" />
