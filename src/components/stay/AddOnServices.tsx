  ...
  return (
    <section id="add-ons" className="scroll-mt-28">
      <div className="mb-6 sm:mb-8">
        <span className="mb-3 inline-flex items-center gap-2 rounded-full border border-[#C49C79]/40 bg-[#C49C79]/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-[#C49C79]">
          <Sparkles size={11} /> Optional Extras
        </span>
        <h2 className="font-display text-2xl font-light text-[#EFE3D5] sm:text-3xl lg:text-4xl">
          Property Services &amp; Add-ons
        </h2>
        <p className="mt-3 max-w-2xl text-sm font-light leading-relaxed text-[#EFE3D5]/70">
          ...
          return (
            <div
              ...
              className={[
                ...
                on
                  ? "border-[#C49C79] shadow-[0_0_0_2px_rgba(196,156,121,0.55)]"
                  : "border-transparent hover:-translate-y-0.5 hover:shadow-lg",
              ].join(" ")}
            >
              {service.popular && (
                <span className="absolute -top-2 right-4 rounded-full bg-[#D4AF37] px-2.5 py-0.5 text-[9px] font-bold uppercase tracking-widest text-[#181113]">
                  Popular
                </span>
              )}
      ...
      {/* ---------- Running total ---------- */}
      <div className="mt-6 rounded-2xl border border-[#C49C79]/25 bg-white/[0.04] p-4 backdrop-blur-sm sm:p-5">
        <div className="flex items-center gap-2">
          <ReceiptText size={16} className="text-[#C49C79]" />
          <h3 className="text-xs font-semibold uppercase tracking-widest text-[#EFE3D5]/80">
            Add-ons Summary
          </h3>
        </div>

        {chosen.length === 0 ? (
          <p className="mt-3 text-sm text-[#EFE3D5]/55">
            ...
            {chosen.map((s) => (
              <li key={s.id} className="flex items-center justify-between gap-3 py-2.5">
                <div className="flex min-w-0 items-center gap-2.5">
                  <AmenityIcon name={s.icon} size={15} className="shrink-0 text-[#C49C79]" />
                  <div className="min-w-0">
                    <span className="block truncate text-sm text-[#EFE3D5]">{s.name}</span>
                    <span className="text-[11px] text-[#EFE3D5]/45">
                ...
                <span className="shrink-0 text-sm font-medium text-[#EFE3D5]">
        ...
        <div className="mt-3 flex items-baseline justify-between border-t border-[#C49C79]/25 pt-3">
          <span className="text-xs font-semibold uppercase tracking-widest text-[#EFE3D5]/80">
            Services Total
          </span>
          <span className="font-display text-xl text-[#C49C79]">
