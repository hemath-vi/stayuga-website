...
const FAQS = [
  {
    q: "What does Stayuga handle?",
    a: "The whole thing — the farmhouse, the food, the décor, the event and the hospitality around it. One contract, one point of contact, one team accountable for the day.",
  },
  {
    q: "How is this different from booking a venue?",
    a: "Most competitors stop at the keys. You would still be sourcing caterers, decorators and coordinators yourself, and carrying the risk if one of them falls through. We manage all of it in-house.",
  },
  {
    q: "Can you host weddings and large gatherings?",
    a: "Yes. Several properties in the collection are cleared for full-scale events, with the lawns and power to match. Share your date and guest count and we will shortlist the ones that fit.",
  },
  {
    q: "How does booking work?",
    a: "Send an enquiry with your dates. A concierge confirms availability, walks you through the property and prices the add-ons you want, then holds the date once you are happy.",
  },
];
    ...
    <div className="min-h-screen bg-cream pt-20 text-ink">
      ...
      <section className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-12 px-6 py-12 md:grid-cols-2">
        ...
        <div>
          ...
          <h2 className="font-display mb-4 text-3xl font-light text-ink">
            A family anniversary, and far too many phone calls.
          </h2>
          <p className="mb-4 text-sm font-light leading-relaxed text-ink-soft">
            The farmhouse was beautiful. Everything else — the caterer, the decorator, the
            coordinator — was six separate conversations and one long, anxious week.
          </p>
          <p className="text-sm font-light leading-relaxed text-ink-soft">
            Stayuga exists so that week never happens to anyone else. One number, one team, and a
            day you get to actually attend.
          </p>
        </div>
      </section>
        ...
        <div className="divide-y divide-line border border-line bg-shell">
          {FAQS.map(({ q, a }) => (
            <details key={q} className="group p-5">
              ...
              <p className="mt-3 text-sm font-light leading-relaxed text-ink-soft">{a}</p>
            </details>
          ))}
        </div>
      </section>
      ...
      {/* ---------------- 6 · CONCIERGE ----------------
          Replaced the stub form that used to sit here (it only fired an
          alert) with the real routes — the working form lives on /contact. */}
